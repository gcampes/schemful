...
export class MigrationTracker {
  private static readonly CONTENT_TYPE_ID = "ctkit_migration_history";
  ...
  async markAsExecuted(execution: MigrationExecution): Promise<void> {
    ...
    const record: Omit<MigrationRecord, "executedAt"> = {
      ...
      ctkitVersion: this.getCtkitVersion(),
        ...
        MigrationTracker.CONTENT_TYPE_ID,
        {
          fields: {
            ...
            description: record.description
              ...
              : undefined,
            ctkitVersion: { "en-US": record.ctkitVersion },
  ...
  async isExecuted(migrationId: string): Promise<boolean> {
    ...
    try {
      ...
      throw new CtkitError(
        ...
        CtkitErrorCode.CONTENTFUL_API_ERROR,
        error
      );
    }
  }
  ...
  async getExecutionHistory(): Promise<MigrationRecord[]> {
    ...
    try {
      ...
      return entries.items.map((entry: any) => ({
        ...
        ctkitVersion: entry.fields.ctkitVersion["en-US"],
  ...
  /**
   * Ensure the migration history content type exists and has the ctkit managed field
   */
  private async ensureMigrationHistoryContentType(): Promise<void> {
    try {
      ...
      // Check if ctkitManaged field exists
      const hasCtkitField = existingContentType.fields.some(
      ...
      if (!hasCtkitField) {
        console.log(
          chalk.blue(
            "🔧 Adding ctkitManaged field to migration history content type..."
          )
        );
        ...
        console.log(
          chalk.green("✅ Added ctkitManaged field to migration history")
        );
      }
    } catch (error: any) {
      if (
