...
describe("Clean Command", () => {
  ...
  it("should handle no content types", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining("✅ No schemful-managed content types found")
    );
      ...
      items: [
        {
          ...
          name: "Regular Type",
          fields: [
            { id: "schemfulManaged", name: "Schemful Managed", type: "Boolean" }
          ],
          unpublish: vi.fn(),
          delete: vi.fn(),
        },
  ...
  it("should handle non-existent content type filter", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining("⚠️  No schemful-managed content types found matching 'nonExistent'")
    );
