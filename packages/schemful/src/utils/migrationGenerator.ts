 ...
 * @throws SchemfulError when Contentful connection fails or schema analysis fails
 */
export async function generateMigrationFromSchemas(
  ...
  if (!schemas || schemas.length === 0) {
    throw new SchemfulError(
      ...
      SchemfulErrorCode.MIGRATION_GENERATION_FAILED
    );
  }
  ...
  for (const schema of sortedSchemas) {
    ...
    if (!existingContentType) {
      ...
      // Check if schemfulManaged field exists, add if missing
      if (!existingFieldMap.has("schemfulManaged")) {
        operations.push({
          type: "createField",
          contentTypeId: schema.id,
          field: {
          ...
          description: `Add schemful management metadata to ${schema.name}`,
        });
      }
      ...
      for (const [fieldId, existingField] of existingFieldMap) {
        // Skip schemfulManaged field - we never delete this
        if (fieldId === 'schemfulManaged') {
          continue;
        }
...
function generateFieldOrderOperations(
  ...
  // Get current field order from Contentful (excluding schemfulManaged)
...
function generateMigrationCode(
  ...
  for (const [contentTypeId, contentTypeOps] of operationsByContentType) {
    ...
    if (hasCreateOp) {
      ...
      // Add schemful managed metadata field first
      codeLines.push("");
      codeLines.push(generateSchemfulManagedFieldCode(contentTypeId));
        ...
        for (const op of fieldOps) {
          if (op.field) {
            ...
            if (op.type === "createField") {
              // Special handling for schemfulManaged field
              if (op.field.id === "schemfulManaged") {
                codeLines.push(
                  generateSchemfulManagedFieldCode(contentTypeId)
                );
              } else {
                codeLines.push(
...
/**
 * Generate schemful managed metadata field creation code
 */
function generateSchemfulManagedFieldCode(contentTypeId: string): string {
  const lines: string[] = [];

  lines.push(`  ${contentTypeId}.createField('schemfulManaged')`);
  lines.push(`    .name('Schemful Managed')`);
