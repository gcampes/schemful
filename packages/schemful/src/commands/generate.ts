...
import { SchemfulError, SchemfulErrorCode, withSpinner } from "../utils";
import {
 ...
 * @throws SchemfulError when generation fails or invalid type provided
 *
 * @example
 * ```typescript
...
export async function generateMigration(
  ...
  if (type === "migration") {
    ...
    throw new SchemfulError(
      ...
      SchemfulErrorCode.INVALID_CONFIGURATION
    );
  }
}
...
async function generateAutoMigration(cwd: string, schemasDir = "schemas"): Promise<void> {
  ...
  try {
    ...
    console.log(
      chalk.yellow(
        "\n💡 Run `schemful migrate` to apply this migration to Contentful"
      )
    );
  } catch (error) {
    console.error(
...
async function generateSchemaFile(name: string, cwd: string): Promise<void> {
  ...
  const schemaTemplate = `import { ContentTypeSchema } from 'schemful';
...
`;
  ...
  console.log(
    chalk.blue(
      `💡 You can now create a migration with: schemful generate migration create_${schemaId}`
    )
  );
}
 ...
 * @param config - Schemful configuration
 */
...
`;
  ...
  console.log(
    chalk.blue(
      `💡 Run the seed with: schemful seed --file seeds/${targetEnv}/${filename}`
    )
  );
}
