...
export async function cleanContentful(
  ...
  try {
    ...
    if (!options.all) {
      // Filter to only show schemful-managed content types (default behavior)
      const schemfulManagedTypes = await filterSchemfulManagedContentTypes(
        contentTypes.items
      );
      targetTypes = schemfulManagedTypes;
      targetDescription = "schemful-managed content types";

      spinner.succeed(
        `Found ${contentTypes.items.length} total content type(s), ${schemfulManagedTypes.length} schemful-managed`
      );

      if (schemfulManagedTypes.length === 0) {
        console.log(
          chalk.green(
            "✅ No schemful-managed content types found in Contentful"
          )
        );
        console.log(
          chalk.gray(
            "   Use the migration generator to create content types with schemful."
          )
        );
        console.log(
          chalk.gray(
      ...
      console.log(
        chalk.red.bold(
          "\n⚠️  --all flag enabled: Will target ALL content types, not just schemful-managed ones!"
        )
      );
    }
...
/**
 * Filter content types to only those managed by schemful
 */
async function filterSchemfulManagedContentTypes(
  contentTypes: any[]
): Promise<any[]> {
  const schemfulManagedTypes: any[] = [];

  for (const contentType of contentTypes) {
    // Check if content type has schemfulManaged field
    const hasSchemfulField = contentType.fields.some(
    ...
    if (hasSchemfulField || isMigrationHistory) {
      schemfulManagedTypes.push(contentType);
    }
  }

  return schemfulManagedTypes;
}
