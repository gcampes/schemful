...
By default, Schemful will **not** overwrite existing schema files. If a file already exists for a content type, it is skipped and a warning is shown.
...
Each pulled content type produces a TypeScript file using the Schemful schema API:
...
`pull` is the recommended way to **adopt Schemful on an existing Contentful space**. Instead of rewriting your content model from scratch:

1. Run `schemful pull` to generate schema files for your current content types.
2. Review the generated files — adjust naming or organization as needed.
3. Run `schemful check` to confirm local and remote are in sync.
